...
bool
Ball::Initialise(Renderer& renderer, BallType _ballType)
{
	ballType = _ballType;
	...
	case BADBALL:
		m_position.x = static_cast<float>(GetRandom(EDGE_LIMIT, SCREEN_WIDTH - EDGE_LIMIT));
		m_position.y = static_cast<float>(SCREEN_HEIGHT - EDGE_LIMIT);
		break;
	case PLAYERBALL:
		m_position.x = static_cast<float>(SCREEN_WIDTH / 2);
		m_position.y = static_cast<float>(SCREEN_WIDTH / 2);
		break;
	case RANDOMBALL:
		m_position.x = static_cast<float>(GetRandom(EDGE_LIMIT, SCREEN_WIDTH - EDGE_LIMIT));
...
void
Ball::ProcessPlayer(InputSystem& inputSystem)
{
	inputSystem.ProcessInput();
	m_position = inputSystem.GetMousePosition();
...
void
Ball::SetPlayerPosition(Vector2 position)
{
...
void
Ball::SetColour(float r, float g, float b)
{
	m_pSprite->SetRedTint(r);
	m_pSprite->SetGreenTint(g);
	m_pSprite->SetBlueTint(b);
}
...
Ball::Shrink()
{
	...
	m_pSprite->SetScale((m_pSprite->GetScale() - sizeChangeScaler));
}
